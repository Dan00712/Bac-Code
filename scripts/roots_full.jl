...
function L(x,y,z, Δω)
    -abs2(Et(x,y,z)) + abs2(αeq_c(x,y,z, Δω)*Ec(x,y,z, Δω)) + 2*real(Et(x,y,z) *αeq_c(x,y,z, Δω)*Ec(x,y,z, Δω))
end
...
for ((i,x), (j, y), (k, z)) in ProgressBar(Iterators.product(enumerate(X), enumerate(Y), enumerate(Z)))
...
for ((i,x), (j, y), (k, z)) in ProgressBar(Iterators.product(enumerate(X), enumerate(Y), enumerate(Z)))
    if isminima(Grid, i, j, k)
        ...
        push!(
              stability,
              if isstable(x,y,z, Δω) 
                :stable else :unstable end
        )
    end
end
...
scatter!(p, Xm.*1e6, Ym.*1e6, Zm.*1e6,
        #color=colors,
        group=group
)
...
savefig(
        p,
        plotsdir("full_roots3d.$(now_nodots()).svg")
)
